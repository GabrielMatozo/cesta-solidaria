...
def main() -> int:
    ...
    for p in catalogo:
        ...
        preco = p.get("preco_atual") or 0
        subtotal = float(qtd) * float(preco)
        itens_cesta.append((p["nome"], qtd, float(preco), subtotal))
    total_cesta = sum(s for *_, s in itens_cesta)

    print("\n===== CESTA DE HOJE (marcas mais baratas do dia) =====", file=sys.stderr)
    for nome, qtd, preco, sub in sorted(itens_cesta, key=lambda x: -x[3]):
        print(f"{nome}: {qtd} x R$ {preco:.2f} = R$ {sub:.2f}", file=sys.stderr)
    ...
    with open("custo_cesta.txt", "w", encoding="utf-8") as fh:
        ...
        for nome, qtd, preco, sub in sorted(itens_cesta, key=lambda x: -x[3]):
            fh.write(f"{nome}: {qtd} x R$ {preco:.2f} = R$ {sub:.2f}\n")
